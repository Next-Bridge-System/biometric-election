...
$application = App\HighCourt::where('id',$application_id)->first();
$number = App\HighCourt::where('id',$application_id)->pluck($number_type)->toArray();
@endphp

@if ($type != 'rcpt_no_hc')
<span> {{$number[0] ?? 'N/A'}} </span>
@if ($number[0] == null)
@if (Auth::guard('admin')->user()->hasPermission('add_'.$type))
<button type="button" class="btn btn-primary float-right btn-xs mr-1 hc_number_btn" data-toggle="modal"
    data-target="#{{$type}}_modal"><i class="fas fa-plus mr-1"></i>Add</button>
@endif
@else
@if (Auth::guard('admin')->user()->hasPermission('edit_'.$type))
<button type="button" class="btn btn-link float-right btn-sm hc_number_btn" data-toggle="modal"
    data-target="#{{$type}}_modal"><i class="far fa-edit"></i></button>
@endif
@endif
...
<div class="modal fade" id="{{$type}}_modal" tabindex="-1" aria-labelledby="{{$type}}_modal_label" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="#" class="hc_number_form" id="{{$type}}_form" method="POST"> @csrf
                <div class="modal-body">
                    <div class="form-group">
                        <input type="text" name="hc_number" id="{{$type}}" class="form-control"
...
@if ($application->rcpt_date == null && Auth::guard('admin')->user()->hasPermission('add_'.$type))
        ...
        <div class="modal-content">
            ...
            <form action="#" class="rcpt_current_date_form" method="POST"> @csrf
                <div class="modal-body">
                    ...
                    <h5 class="modal-title" id="add_rcpt_current_date_modal_label">Are you sure, you want to add RCPT -
