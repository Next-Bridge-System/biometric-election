...
<head>
    ...
    <title>Lower Court License Report - {{ date('F d, Y - H:i A')}}</title>
...
<body>
    ...
    <div style="text-align: center">
        <h3>PBC - LOWER COURT LICENSE REPORT</h3>
    ...
    <table>
        ...
        <tbody style="text-align: center">
            ...
            <tr>
                ...
                <td>
                    {{ $application->lawyer_name ? $application->lawyer_name : '-' }} /
                    {{$application->father_name ? $application->father_name : '-' }}
                </td>
                <td>{{ getDateFormat($application->lc_date)}} </td>
                <td>{{ str_replace('BAR ASSOCIATION', '', getBarName($application->voter_member_lc)) ?? '-' }}</td>
                <td>{{ $application->mobile_no ? $application->mobile_no : '-' }} </td>
                <td>{{$application->cnic_no ?? '-'}}</td>
                <td>{{ $application->reg_no_lc ?? '-' }}</td>
                <td>{{ $application->plj_no_lc ?? '-' }}</td>
                <td>{{ $application->bf_no_lc ?? '-' }}</td>
                <td>{{ $application->gi_no_lc ?? '-' }}</td>
                @endforeach
                @endforeach
        </tbody>
    </table>
    ...
    <div class="row" style="font-size:17px">
        ...
        <div class="column" style="text-align: center; border:1px solid black">
            <div>
                <h4>{{ $application->lawyer_name}} SDW/o {{ $application->father_name}}</h4>
                <strong>Address:</strong> {{ getLcPostalAddress($application->id) }}, {{ $application->mobile_no}}
            </div>
            <div>
                <strong>License No:</strong>{{ $application->license_no_lc ?? 'N/A' }},
                <strong>Legder No:</strong>{{ $application->reg_no_lc ?? 'N/A' }}, <br>
                <strong>PLJ No:</strong>{{ $application->plj_no_lc ?? 'N/A' }}
                <strong>BF No:</strong>{{ $application->bf_no_lc ?? 'N/A' }},<br>
                <strong>GI No:</strong>{{ $application->gi_no_lc ?? 'N/A' }},
                <strong>RCPT No:</strong>{{ $application->rcpt_no_lc ?? 'N/A' }}
            </div>
        </div>
