...
<button type="button" class="btn btn-primary float-right btn-sm m-1" data-toggle="modal" data-target="#notesModal">
    <i class="far fa-copy mr-1"></i>Additional Notes <span
        class="badge badge-warning ml-1">{{$application->additional_notes->count()}}</span>
</button>
                ...
                <div class="form-group col-md-12">
                    <form action="#" id="notes_form" method="POST"> @csrf
                        <div class="form-group" id="notes">
                            <textarea type="text" name="intimation_notes" id="intimation_notes"
                        ...
                        <div class="card-header" id="headingOne{{$note->id}}">
                            <h5 class="mb-0">
                                <button class="btn btn-link" data-toggle="collapse"
                                    ...
                                    <b>Note # {{$loop->iteration}}: </b> Submitted
                                    By {{getAdminName($note->admin_id)}} at {{$note->created_at}}
                                </button>
                            </h5>
                        </div>
