...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>Lower Court Application - Payment</h1>
            </div>
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="{{route('lower-court.show',$lc->id)}}"
                                    ...
                                    <form action="#" method="POST" id="payment_form" enctype="multipart/form-data">
                                        ...
                                        <tr>
                                            ...
                                            <td rowspan="2" class="text-center">
                                                ...
                                                <a href="{{route('lower-court.destroy.voucher')}}"
                                        ...
                                        <tr>
                                            <th>Voucher Type:</th>
                                            <td>{{ getLcVoucherName($payment->voucher_type) ?? '- -' }}</td>
                                        </tr>
                                        <tr>
    ...
    $(document).ready(function(){
      $("#payment_form").on("submit", function(event){
          ...
          $.ajax({
              method: "POST",
              data: formData,
              url: '{{route('lower-court.payment', [$lc->id,$payment->voucher_type])}}',
              processData: false,
              contentType: false,
              cache: false,
              beforeSend: function(){
              ...
              success: function (response) {
                if (response.status == 1) {
                    window.location.href = '{{route('lower-court.show', $lc->id)}}';
                }
              },
...
<script>
    ...
    voucher.setOptions({
      server: {
          url: '{{route('lower-court.payment.voucher')}}',
          headers: {
