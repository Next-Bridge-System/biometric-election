                ...
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item">
                        <div class="">
                            ...
                            <span>
                                Candidate: {{$application->lawyer_name}}
                            </span> <br>
                            <a href="{{route('lower-court.show',$application->id)}}" class="btn btn-primary btn-xs m-1">
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="card card-success">
                    ...
                    @include('admin.lower-court.partials.steps')
                    {{-- The portion where the forms will be rendered --}}
                    <div id="step-render">
                        <form class="steps-form" action="#" method="POST"
                            data-action="{{ route('lower-court.create-step-1',$application->id) }}"
                                    ...
                                    <div class="row" id="application_form">
                                        ...
                                        <div class="form-group col-md-6">
                                            ...
                                            <div class="input-group date" id="intimation_date"
                                                ...
                                                <input type="text"
                                                    value="{{getDateFormat($application->intimation_date)}}"
                                                    class="form-control datetimepicker-input"
                                                    data-target="#intimation_date" name="intimation_date"
...
<script>
    jQuery(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response.status === 1) {
                        ...
                        goToStep('{{ route('lower-court.create-step-4',$application->id) }}', 4)
                    }
        ...
        $(document).on("click", '#saveAndNext', function (event) {
            ...
            if (target === '#academic_group_modal') {
                $('#academic_group_modal').modal('show');
            }
            else {
                $(".custom-loader").removeClass('hidden');
                $.get("{{ route('lower-court.create-step-5',$application->id) }}", function (data, status) {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.get(event.dataset.action, function (data, status) {
                        var action = $('#admin_lc_create_step_4_form').data('action');
                        goToStep(action, 4)
                    });
                }
            })
    ...
    $(document).on('submit', '.exemption_form', function (e) {
        e.preventDefault();
        var url = '{{ route("lower-court.create-step-4.exemption", ":id") }}';
