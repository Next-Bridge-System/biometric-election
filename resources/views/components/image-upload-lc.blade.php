...
<div class="{{ isset($is_detail_profile) &&  $is_detail_profile == 'true' ? '' : 'col-md-4' }} form-group">
    ...
    <a href="javascript:void(0)" @if (Route::currentRouteName()=='frontend.lower-court.create-step-5' ||
        Route::currentRouteName()=='frontend.lower-court.create-step-6' )
        data-action="{{route('frontend.lower-court.destroy.file', $name)}}" @else
        data-action="{{route('lower-court.destroy.file', $name)}}" @endif
        class="btn btn-danger btn-xs col-md-12 mt-2 mb-4" onclick="removeImage(this,6)">Remove</a>
    @else
...
<script>
    ...
    if (currentRouteName == 'frontend.lower-court.create-step-1') {
        var url = '{{ route("frontend.lower-court.upload.file", ":slug") }}';
    } else {
